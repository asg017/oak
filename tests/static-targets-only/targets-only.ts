...
test("static-targets-only", async t => {
  ...
  await oak_static({ filename: env("Oakfile"), targets: ["a"] });
  ...
  await oak_static({ filename: env("Oakfile"), targets: ["d"] });
  ...
  await oak_static({ filename: env("Oakfile"), targets: ["h"] });
